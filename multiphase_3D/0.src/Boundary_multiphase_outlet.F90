        ...
        do j=1,ny
            do i=1,nx
              if(walls(i,j,k)==0)then  
                ...
                phi(i,j,nz+1) = ((phi_convec_bc(i,j) + u_convec*phi(i,j,nz))*temp) 
                ...
                f6(i    , j    , nz + 1) = ((f_convec_bc(i,j,6) + u_convec * f6(i    , j    , nz + 1 - 1)) * temp) 
                f13(i - 1, j    , nz + 1) = ((f_convec_bc(i,j,13) + u_convec * f13(i - 1, j    , nz + 1 - 1)) * temp) 
                f14(i + 1, j    , nz + 1) = ((f_convec_bc(i,j,14) + u_convec * f14(i + 1, j    , nz + 1 - 1)) * temp) 
                f17(i    , j - 1, nz + 1) = ((f_convec_bc(i,j,17) + u_convec * f17(i    , j - 1, nz + 1 - 1)) * temp) 
                f18(i    , j + 1, nz + 1) = ((f_convec_bc(i,j,18) + u_convec * f18(i    , j + 1, nz + 1 - 1)) * temp) 

                g6(i    , j    , nz + 1) = ((g_convec_bc(i,j,6) + u_convec * g6(i    , j    , nz + 1 - 1)) * temp) 
                g13(i - 1, j    , nz + 1) = ((g_convec_bc(i,j,13) + u_convec * g13(i - 1, j    , nz + 1 - 1)) * temp) 
                g14(i + 1, j    , nz + 1) = ((g_convec_bc(i,j,14) + u_convec * g14(i + 1, j    , nz + 1 - 1)) * temp) 
                g17(i    , j - 1, nz + 1) = ((g_convec_bc(i,j,17) + u_convec * g17(i    , j - 1, nz + 1 - 1)) * temp) 
                g18(i    , j + 1, nz + 1) = ((g_convec_bc(i,j,18) + u_convec * g18(i    , j + 1, nz + 1 - 1)) * temp) 
        ...
        do j=1,ny
            do i=1,nx
              if(walls(i,j,k)==0)then  
                ...
                f5(i,j,nz) = (( f_convec_bc(i,j,6) + u_convec*f5(i,j,nz-1))*temp  )           
                f11(i,j,nz) = (( f_convec_bc(i,j,14) + u_convec*f11(i,j,nz-1))*temp  )
                f12(i,j,nz) = (( f_convec_bc(i,j,13) + u_convec*f12(i,j,nz-1))*temp  )             
                f15(i,j,nz) = (( f_convec_bc(i,j,18) + u_convec*f15(i,j,nz-1))*temp  )
                f16(i,j,nz) = (( f_convec_bc(i,j,17) + u_convec*f16(i,j,nz-1))*temp  )
                
                g5(i,j,nz) = (( g_convec_bc(i,j,6) + u_convec*g5(i,j,nz-1))*temp  )           
                g11(i,j,nz) = (( g_convec_bc(i,j,14) + u_convec*g11(i,j,nz-1))*temp  )
                g12(i,j,nz) = (( g_convec_bc(i,j,13) + u_convec*g12(i,j,nz-1))*temp  )              
                g15(i,j,nz) = (( g_convec_bc(i,j,18) + u_convec*g15(i,j,nz-1))*temp  )
                g16(i,j,nz) = (( g_convec_bc(i,j,17) + u_convec*g16(i,j,nz-1))*temp  )
...
subroutine outlet_Zou_He_pressure_BC_before_odd    !before streaming type BC
    ...
    if(idz==npz-1)then
        !$omp parallel
        !$omp do private(tmp1,tmp2,tnx,tny,i)
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~_openacc
        !$acc kernels present(f0,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,f17,f18,&
        ...
        do j=1,ny
            do i=1,nx
              if(walls(i,j,k)==0)then  
                ...
                f6(i,j,nz+1)     = (f5(i,j,nz-1)-1.0d0/3.0d0*tmp1) 
                f13(i-1,j,nz+1) = (f12(i+1,j,nz-1)-0.166666666666666667d0*tmp1 - tnx) 
                f14(i+1,j,nz+1) = (f11(i-1,j,nz-1)-0.166666666666666667d0*tmp1 + tnx) 
                f17(i,j-1,nz+1) = (f16(i,j+1,nz-1)-0.166666666666666667d0*tmp1 - tny) 
                f18(i,j+1,nz+1) = (f15(i,j-1,nz-1)-0.166666666666666667d0*tmp1 + tny) 
                ...
                g6(i,j,nz+1)     = (g5(i,j,nz-1)-1.0d0/3.0d0*tmp2) 
                g13(i-1,j,nz+1) = (g12(i+1,j,nz-1)-0.166666666666666667d0*tmp2 - tnx) 
                g14(i+1,j,nz+1) = (g11(i-1,j,nz-1)-0.166666666666666667d0*tmp2 + tnx) 
                g17(i,j-1,nz+1) = (g16(i,j+1,nz-1)-0.166666666666666667d0*tmp2 - tny) 
                g18(i,j+1,nz+1) = (g15(i,j-1,nz-1)-0.166666666666666667d0*tmp2 + tny) 
              endif
            enddo
        enddo
        !$acc end kernels
        ...
        do j=1,ny
            do i=1,nx
              if(walls(i,j,k)==0)then  
                ...
                f5(i,j,nz) = (f6(i,j,nz)-1.0d0/3.0d0*tmp1)
                f11(i,j,nz) = (f14(i,j,nz)-0.166666666666666667d0*tmp1 + tnx)
                f12(i,j,nz) = (f13(i,j,nz)-0.166666666666666667d0*tmp1 - tnx)
                f15(i,j,nz) = (f18(i,j,nz)-0.166666666666666667d0*tmp1 + tny)
                f16(i,j,nz) = (f17(i,j,nz)-0.166666666666666667d0*tmp1 - tny)
                ...
                g5(i,j,nz) = (g6(i,j,nz)-1.0d0/3.0d0*tmp2)
                g11(i,j,nz) = (g14(i,j,nz)-0.166666666666666667d0*tmp2 + tnx)
                g12(i,j,nz) = (g13(i,j,nz)-0.166666666666666667d0*tmp2 - tnx)
                g15(i,j,nz) = (g18(i,j,nz)-0.166666666666666667d0*tmp2 + tny)
                g16(i,j,nz) = (g17(i,j,nz)-0.166666666666666667d0*tmp2 - tny)
              endif
            enddo
        enddo
        !$acc end kernels
