        ...
        DO j = iymin, iymax
            ...
            do i = ixmin, ixmax
                if (walls(i, j, k) == 0) then
                    ...
                    den = rho1 + rho2
                    ux1 = ft1 - ft2 + ft7 - ft8 + ft9 - ft10 + ft11 - ft12 + ft13 - ft14 + 0.5d0*fx
                    uy1 = ft3 - ft4 + ft7 + ft8 - ft9 - ft10 + ft15 - ft16 + ft17 - ft18 + 0.5d0*fy
                    uz1 = ft5 - ft6 + ft11 + ft12 - ft13 - ft14 + ft15 + ft16 - ft17 - ft18 + 0.5d0*fz
...
m_3pixx = m_3pixx - s_pi*(m_3pixx - mrt_omega_xx*(3d0*ux1*ux1 - u2)) + (1d0 - 0.5d0*s_pi)*(-2d0*fx*ux1 + fy*uy1 + fz*uz1)         !m10
   ...
   m_piww = m_piww - s_pi*(m_piww - mrt_omega_xx*(uy1*uy1 - uz1*uz1)) + (1d0 - 0.5d0*s_pi)*(-fy*uy1 + fz*uz1)                   !m12
         ...
         m_pzx = m_pzx - s_nu*(m_pzx - (ux1*uz1)) + (1d0 - 0.5d0*s_nu)*(fx*uz1 + fz*ux1)                                        !m15
                    ...
                    g1t1 = tmp1*ft1 + w_equ_1*tmp*(cnx)
                    g1t2 = tmp1*ft2 + w_equ_1*tmp*(-cnx)
                    g1t3 = tmp1*ft3 + w_equ_1*tmp*(cny)
                    g1t4 = tmp1*ft4 + w_equ_1*tmp*(-cny)
                    g1t5 = tmp1*ft5 + w_equ_1*tmp*(cnz)
                    g1t6 = tmp1*ft6 + w_equ_1*tmp*(-cnz)
                    g1t7 = tmp1*ft7 + rk_weight2*tmp*(cnx + cny)
                    g1t8 = tmp1*ft8 + rk_weight2*tmp*(-cnx + cny)
                    g1t9 = tmp1*ft9 + rk_weight2*tmp*(cnx - cny)
                    g1t10 = tmp1*ft10 + rk_weight2*tmp*(-cnx - cny)
                    g1t11 = tmp1*ft11 + rk_weight2*tmp*(cnx + cnz)
                    g1t12 = tmp1*ft12 + rk_weight2*tmp*(-cnx + cnz)
                    g1t13 = tmp1*ft13 + rk_weight2*tmp*(cnx - cnz)
                    g1t14 = tmp1*ft14 + rk_weight2*tmp*(-cnx - cnz)
                    g1t15 = tmp1*ft15 + rk_weight2*tmp*(cny + cnz)
                    g1t16 = tmp1*ft16 + rk_weight2*tmp*(-cny + cnz)
                    g1t17 = tmp1*ft17 + rk_weight2*tmp*(cny - cnz)
                    g1t18 = tmp1*ft18 + rk_weight2*tmp*(-cny - cnz)
    ...
    RETURN
end subroutine kernel_odd_color
